...
func configureResolver(port int) (func(), error) {
	...
	if err := exec.Command("resolvectl", "dns", "@DEFAULT", addr).Run(); err != nil {
		return nil, err
	}
	if err := exec.Command("resolvectl", "domain", "@DEFAULT", "~p2p").Run(); err != nil {
		exec.Command("resolvectl", "revert", "@DEFAULT").Run()
		return nil, err
	}
	return func() {
		exec.Command("resolvectl", "revert", "@DEFAULT").Run()
